...
var RateLimiter = require('app_waring/cartridge/scripts/util/RateLimiter');
...
function handleOAuthReentry() {
    ...
    if (!profile) {
        Transaction.wrap(function () {
            ...
            // Google comes with a 'given_name' and 'family_name' property that holds first and last name.
            if (extProfile.given_name || extProfile.family_name) {
                firstName = extProfile.given_name;
                lastName = extProfile.family_name;
            }else {
...
/** @deprecated This is only kept for compatibility reasons, use {@link module:controllers/Login~handleOAuthReentry|handleOAuthReentry} instead */
exports.OAuthReentryTwitter    = guard.ensure(['https','get'], handleOAuthReentry);
/** @deprecated This is only kept for compatibility reasons, use {@link module:controllers/Login~handleOAuthReentry|handleOAuthReentry} instead */
