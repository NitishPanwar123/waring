 ...
 */
/*This function is modified for rendering cart only when user if logged in else it will show login page*/
function show() {
        var cartForm = app.getForm('cart');
        app.getForm('login').invalidate();
        app.getForm('ShippingForm').clear();
        cartForm.get('shipments').invalidate();
    
        app.getView('Cart', {
            // ContinueURL: URLUtils.https('Cart-HandleForm'),
            cart: app.getModel('Cart').get(),
            RegistrationStatus: false
        }).render('checkout/cart/cart');
    }
    // function handleForm() {
    //     app.getForm('ShippingForm').handleAction({
    //         cancel: function () {
    //             app.getForm('helloform').clear();
    //             response.redirect(URLUtils.https('Cart-Show'));
    //         },
    //         submit: function () {
    //             app.getForm('helloform');
    //         }
    //     });
    // }
/**
...
function submitForm() {
    ...
    formResult = cartForm.handleAction({
        ...
        'checkoutCart': function () {
            ...
            // app.getForm('ShippingForm').submit;
            // var t=0;
            // var g=app.getForm('ShippingForm');
            ...
            if (product.isBundle()) {
                ...
                for (j = 0; j < childPids.length; j += 1) {
                    ...
                    if (childProduct) {
                        ...
                        var UpdateProductOptionSelections = require('app_waring/cartridge/scripts/cart/UpdateProductOptionSelections');
