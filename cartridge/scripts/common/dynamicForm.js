 ...
 */
function getFieldContext(field, fieldData,formElementName) {
	const context = {};
	let type;
	...
	context.formElementName = formElementName;
...
module.exports.getFields = function (formObject, formData) {
	...
	for (var formElementName in formObject) {
		Logger.info('Form Element Name');
		Logger.info(formElementName);
		...
		if (formElement instanceof FormField) {
			...
			fields.push(getFieldContext(formElement, fieldData,formElementName));
			...
			if (fieldData && Array.isArray(fieldData)) {
				for (var i = 0; i < fieldData.length; i++) {
					...
					fields.push(getFieldContext(childFieldElement, childField,formElementName));
				}
			}
		}
	}
