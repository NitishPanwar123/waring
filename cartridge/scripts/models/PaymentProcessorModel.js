...
PaymentProcessorModel.authorize = function (order, paymentInstrument) {
    ...
    if(processor.ID == "PAYPAL_EXPRESS"){
        if (HookMgr.hasHook('app.payment.processor.' + processor.ID+"_USWARING")) {
            return HookMgr.callHook('app.payment.processor.' + processor.ID+"_USWARING", 'Authorize', {
                Order: order,
