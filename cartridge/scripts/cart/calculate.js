...
exports.calculate = function (basket) {
    ...
    //try{
		//var UPSResult = require('*/cartridge/controllers/UPS').GetCostMap();
        //var costMap = UPSResult.UPSCostMap;
        //let keys = Array.from( costMap.keys());
        //dw.system.Logger.warn('costmap' +keys);
		/*if(!empty(costMap)) {
			for(var i = 0; i < basket.shipments.length; i++) {
				
				if(!empty(basket.shipments[i].shippingMethod)) {
					var serviceCode = basket.shipments[i].shippingMethod.custom.upsServiceCode;
					var isUPS = !empty(serviceCode);
					
					if(isUPS) {
                        var serviceCost = costMap[serviceCode.toString()];
                        dw.system.Logger.warn('serviceCost' +serviceCost);
						if(basket.shipments[i].productLineItems.length == 0) {
							basket.shipments[i].shippingLineItems[0].setPriceValue(0);
						} else if(!empty(serviceCost)) {
                            var chargeAmount : Number = parseFloat(serviceCost);
                            
							basket.shipments[i].shippingLineItems[0].setPriceValue(chargeAmount);
						} else {
							Logger.error('Service Cost could not be retrieved from cost map.  Using built in shipping costs.');
						}
					} else {
						Logger.info('Shipping Method ' + basket.shipments[i].shippingMethod.ID + ' is not UPS.  Using built in shipping costs.');
					}
				}    		
			}
		} else {
			Logger.info('UPS-GetCostMap returned an empty map!  Using built in shipping costs only.');
		}
    }catch(ex){}*/
