...
var Countries = require('app_waring/cartridge/scripts/util/Countries');
var BasketMgr = require('dw/order/BasketMgr');
/* Script Modules */
var app = require('~/cartridge/scripts/app');
var guard = require('~/cartridge/scripts/guard');
var acoustic = require('~/cartridge/scripts/acoustic/acoustic');

function call_acoustic(cart){
    if (app.getForm('ShippingForm').object.acoustic2.checked === true) {
        var email = app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value;
        acoustic.acoustic_addRecipient(email);
    }
}
...
function initAddressForm(cart) {
    ...
    try {
        var tt=app.getForm('ShippingForm').object.forPurpose.value;
        var t1=0;
        Transaction.wrap(function () {
           basket.custom.OrderPurpose = app.getForm('ShippingForm').object.forPurpose.value;
        });
   } catch (error) {
       Transaction.wrap(function () {
        basket.custom.OrderPurpose="Faild";
       });
   }
}
...
function billing() {
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            Transaction.wrap(function () {
                var cart = app.getModel('Cart').get();
                if (!resetPaymentForms() || !validateBilling() || !handleBillingAddress(cart) ||  handlePaymentSelection(cart).error) {
                    returnToForm(cart);
                } else {
